                ...
                <div class="card-header d-flex justify-content-between align-items-center">
                    <div class="d-flex flex-wrap align-items-center gap-2">
                        ...
                        <form method="GET" action="{{ route('rawatinap.pasien') }}"
                            ...
                            <a href="{{ route('rawatinap.pasien.exportPdf', request()->query()) }}"
                                class="btn btn-outline-danger btn-sm d-flex align-items-center gap-1" target="_blank"
                                style="margin-left: 5px;">
                                <i class="fas fa-file-pdf"></i> Export PDF
                            </a>
                            <a href="{{ route('rawatinap.pasien.exportExcel', request()->query()) }}"
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.body.addEventListener('click', function(e) {
            if (e.target.classList.contains('btn-riwayat')) {
                ...
                fetch(`/rawatinap/riwayat-berobat/${noRekamMedis}/dates?type=rawatinap`, {
                        credentials: 'include'
                    })
        ...
        document.getElementById('riwayatList').addEventListener('click', function(ev) {
            if (ev.target.classList.contains('btnLihat')) {
                ...
                const hasilPeriksaDetail = document.getElementById('hasilPeriksaDetail');
                const riwayatList = document.getElementById('riwayatList');
                fetch(`/rawatinap/riwayat-berobat/${noRekamMedis}/${tanggal}?source=${ev.target.getAttribute('data-source')}`, {
                        credentials: 'include'
                    })
                    .then(response => {
                        if (!response.ok) {
        ...
        document.getElementById('riwayatList').addEventListener('click', function(ev) {
            if (ev.target.classList.contains('btnLihat')) {
                ...
                const hasilPeriksaDetail = document.getElementById('hasilPeriksaDetail');
                const riwayatList = document.getElementById('riwayatList');
                fetch(`/rawatinap/riwayat-berobat/${noRekamMedis}/${tanggal}`)
                    .then(response => {
                        if (!response.ok) {
                            throw new Error('Data hasil periksa tidak ditemukan');
                        }
        ...
        $('#btnSimpan').on('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: `/pasienrawatinap/update/${$('#modalNoRekamMedis').val()}`,
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchInput.addEventListener('input', function() {
            ...
            debounceTimeout = setTimeout(() => {
                ...
                fetch(`{{ route('rawatinap.pasien') }}?search=${encodeURIComponent(query)}`, {
                        headers: {
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '#btnSimpanTambah', function(event) {
            ...
            $.ajax({
                url: "{{ route('rawatinap.pasien.store') }}",
                type: "POST",
                data: data,
                success: function(response) {
                    toastr.options = {
                    ...
                    // Tutup modal tambah pasien setelah berhasil simpan
