...
<button class="btn btn-info btn-sm btn-selengkapnya" data-bs-toggle="modal"
    ...
    data-no-rekam-medis="{{ $pasien->pasien ? $pasien->pasien->no_rekam_medis : '' }}">
    Selengkapnya
</button>
                                                <span style="display:none;"
                                                    class="debug-no-rekam-medis">{{ $pasien->pasien ? $pasien->pasien->no_rekam_medis : 'NULL' }}</span>
                                                <button class="btn btn-warning btn-sm ms-1 btn-riwayat-analisa"
                                                    data-bs-toggle="modal" data-bs-target="#modalRiwayatAnalisa"
                                                    data-pasien-id="{{ $pasien->pasien_id }}"
                                                    data-nama-pasien="{{ $pasien->nama_pasien }}">Riwayat Analisa</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
...
document.addEventListener('DOMContentLoaded', function () {
    var modalDetailPasien = document.getElementById('modalPasienDetail');
...
console.log('DEBUG: noRekamMedis:', noRekamMedis);
...
<div class="modal fade" id="modalRiwayatAnalisa" tabindex="-1" aria-labelledby="modalRiwayatAnalisaLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered" style="max-width: 100%;">
        <div class="modal-content" style="overflow-x: hidden;">
            <div class="modal-header d-flex justify-content-between">
                <h3 class="modal-title" id="modalRiwayatAnalisaLabel"><strong>Riwayat Analisa <span
                            id="riwayat_nama_pasien_display"></span></strong></h3>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-3" style="max-height: 400px; overflow-y: auto;">
            ...
            $.get('/rawatinap/hasilanalisa/riwayat/' + encodeURIComponent(pasienId), function (res) {
                if (res.success && res.data) {
                    ...
                    function arr(val) {
                        ...
                        if (typeof val === 'string') try { var arr = JSON.parse(val); if (Array.isArray(arr)) return arr.join(', '); } catch (e) { };
                        return val || '-';
                    }
                    var html = '';
