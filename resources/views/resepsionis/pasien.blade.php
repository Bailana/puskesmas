...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Handle Riwayat Berobat button click in resepsionis view
        document.querySelectorAll('button[data-bs-target="#modalRiwayatBerobat"]').forEach(button => {
            button.addEventListener('click', function () {
                ...
                riwayatList.appendChild(loadingIndicator);

                // Fetch riwayat berobat dates by pasien no_rekam_medis or id if available
                fetch(`/resepsionis/riwayat-berobat/${noRekamMedis}`)
                    .then(response => {
                        if (!response.ok) {
                            return response.text().then(text => {
                                throw new Error(text ||
                                    'Gagal mengambil data riwayat berobat');
                            });
                        }
                    ...
                    .then(dates => {
                        ...
                        riwayatList.removeChild(loadingIndicator);

                        if (!dates || dates.length === 0) {
                            riwayatList.innerHTML =
                                '<p>Riwayat berobat pasien tidak tersedia.</p>';
                            // Hide detail section if no data
                            document.getElementById('hasilPeriksaDetail').style.display =
                                'none';
                        } else {
                            ...
                            riwayatList.querySelectorAll('.btnLihat').forEach(btnLihat => {
                                btnLihat.addEventListener('click', function () {
                                    ...
                                    fetch(
                                            `/resepsionis/hasil-periksa-detail/${noRekamMedis}/${tanggal}`
                                        )
                                        .then(response => {
                                            if (!response.ok) {
                                                throw new Error(
                    ...
                    .catch(error => {
                        if (error.message.includes('403')) {
                            riwayatList.innerHTML =
                                '<p>Anda tidak memiliki akses untuk melihat riwayat berobat pasien.</p>';
                        } else {
